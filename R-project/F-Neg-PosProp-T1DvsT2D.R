...
c1 <- 5
c2 <- 6
t1d <- read.csv(path1)[, c(c1, c2)]
t2d <- read.csv(path2)[, c(c1, c2)]
...
if(ncol(combined_data) == 3) {
  ...
  classification_cols <- colnames(t1d_wide)
  ...
  combined_data_long <- rbind(t1d_long, t2d_long)
}
...
if("all" %in% all_classifications) {
  ...
  t1d_total_raw <- read.csv(path1)[2, c2]
  t2d_total_raw <- read.csv(path2)[2, c2]
...
for (class in classifications) {
  ...
  t2d_data <- proportion_data[proportion_data$classification == class & proportion_data$group == "T2D", ]
  
  if (nrow(t1d_data) > 0 && nrow(t2d_data) > 0) {
    t1d_count <- t1d_data$value[1]
    t1d_total <- t1d_data$total[1]
    t2d_count <- t2d_data$value[1]
    t2d_total <- t2d_data$total[1]
    
    tryCatch({
      # Determine test direction based on classification
      if (grepl("Negative", class, ignore.case = TRUE)) {
        # For Negative: test if T1D > T2D
        prop_test <- prop.test(c(t1d_count, t2d_count), c(t1d_total, t2d_total), 
                              alternative = "two.sided", correct = TRUE)
      } else {
        # For positive classifications: test if T1D < T2D
        prop_test <- prop.test(c(t1d_count, t2d_count), c(t1d_total, t2d_total), 
...
p <- ggplot(filtered_summary_data, aes(x = classification, y = mean_value, fill = group)) +
  ...
  scale_fill_manual(values = c("T1D" = "lightcoral", "T2D" = "gray40")) +
  labs(
    title = "miR-155 Expression in Pancreatic Cells (T1D vs T2D)",
    x = NULL,
    y = "Percentage (%)",
    fill = "Group"
  ) +
...
# Print results
cat("\nDirectional proportion test results (T1D vs T2D):\n")
cat("For Negative: Significant stars indicate T1D has significantly LOWER proportions than T2D\n")
cat("For 1+, 2+, 3+: Significant stars indicate T1D has significantly HIGHER proportions than T2D\n")
